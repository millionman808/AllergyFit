            ...
            ScrollView {
                VStack(spacing: Theme.Metrics.spacing) {
                    ...
                    if !reminders.authorized {
                        Text("If reminders don't appear, enable notifications for SafeFuel in the Settings app.")
...
struct SubscriptionView: View {
    ...
    private var header: some View {
        VStack(spacing: 10) {
            ...
            Text("SafeFuel Premium").font(Theme.Fonts.title).foregroundStyle(Theme.Colors.textPrimary)
...
enum AllergistReport {
    static func makePDF(name: String, allergens: [String], goal: String,
        ...
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("SafeFuel-Report.pdf")
        do {
            try renderer.writePDF(to: url) { ctx in
                ...
                var y: CGFloat = 48
                "SafeFuel — Allergen & Nutrition Summary".draw(at: CGPoint(x: 48, y: y), withAttributes: title); y += 40
                ...
                let disclaimer = "This report is generated by SafeFuel, a lifestyle tool and not a medical device. Allergen entries are self-reported or scanned by the user and must be confirmed by a licensed allergist. Always verify physical food labels and carry prescribed epinephrine."
